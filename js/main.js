...
async function userFetchWeather(){
  ...
  let url = `http://api.openweathermap.org/geo/1.0/direct?q=${city},${state},US&appid=a6211e4c7a048dd89e23d0e4ef2c9326&units=imperial`
  try{
    let response = await fetch(url)
    return await response.json();
  }catch(error){
...
async function getWeatherData() {
  ...
  document.querySelector('.maindisplay__text').innerText =`City Name: ${information[0].name}`
